...
confidence and a free-text reasoning string. Pydantic schema, prompt, pair
...
class ReplicabilityAssessment(BaseModel):
    ...
    scores: FacetScore = Field(
        description="Ordinal reconstructability per facet in {1,2,3,4}: "
...
def main() -> int:
    ...
    for i, filename in enumerate(files, start=1):
        ...
        if not reproduction_pdf.is_file():
            print(f"[{i}/{len(files)}] missing reproduction report: {filename}", flush=True)
            continue

        print(f"[{i}/{len(files)}] scoring {filename}", flush=True)
        try:
            result = analyst.assess(ORIGINAL_DIR / filename, reproduction_pdf, filename=filename)
            json_path.write_text(result.model_dump_json(indent=2), encoding="utf-8")
        except Exception as exc:
            print(f"  ERROR on {filename}: {type(exc).__name__}: {exc}", flush=True)
        time.sleep(2)

    print("\n" + json.dumps(analyst.usage_summary(), indent=2))
    return 0
